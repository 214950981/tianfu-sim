...
export interface TerminalTransitionRecord {
  ...
  sequence: number;
  /** UI04E — NEXT_LIFE only: the freshly minted bootstrap key of the next life, so an exact retry after a post-commit timeout recovers the same `nextBootstrapId` without minting a second life. */
  nextBootstrapId?: string;
  /** UI04E — NEXT_LIFE only: the freshly minted run id of the next life. */
  nextRunId?: string;
}
