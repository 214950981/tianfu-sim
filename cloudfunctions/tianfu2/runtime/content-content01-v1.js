...
//
// Source of truth: packages/content/src/content01-v1.ts
// Source sha256:   c5590e8cdb28ba64d2151eb4ffde4469e5ff148dc92c89a40079b2d1b684b7c5
...
var { sealContentPack, REPEAT_SENSITIVE_EFFECT_OPS } = require("./content-registry.js");
...
const oncePerRun                                           = { maxOccurrences: 1 };
const spaced = (minNodesBetween        )                                           => ({ minNodesBetween });
const repeatable = (spec      )          => spec.cooldown !== undefined && spec.cooldown.maxOccurrences !== 1;
const declared = (spec      ) => (effects              )               => effects.map((effect) => repeatable(spec) && REPEAT_SENSITIVE_EFFECT_OPS.has(effect.op) ? { ...effect, repeatBehavior: "allow-cumulative" }               : effect);

const CONTENT01_VERSION = "content01.v1";
const CONTENT01_ZH_CN                         = {};
const text = (key        , value        )         => { CONTENT01_ZH_CN[key] = value; return key; };
const buildEffect = (build                            , amount = 900)             => ({ op: "ADD_BUILD_EVIDENCE", buildId: `build.${build}`, amount, reasonTag: `content01.${build}` });
const participant = (slot        , kind                      , id        )              => kind === "core" ? { slot, source: { kind, npcDefinitionId: id } } : { slot, source: { kind, archetypeId: id } };
const core = (id        )              => participant("actor", "core", `content01.npc.${id}`);
const generated = (id        )              => participant("actor", "generated", `content01.archetype.${id}`);
...
const closureEffect = (op                                  )               => [{ op, triggeringCause: true }];
...
function choices(spec      )                     {
  ...
  const close = (op                                  )               => spec.closure === undefined ? [] : declare(closureEffect(op));
  const safePrimary               = declare(spec.effects ?? (spec.build === undefined ? [{ op: "ADD_RESOURCE", key: "spiritStone", amount: 2 }] : [buildEffect(spec.build)]));
  if (spec.origins !== undefined) return [
    ...spec.origins.map((origin, index)                   => ({
  ...
  if (spec.risk !== undefined) return [
    { id: "take-risk", scope: "core", labelKey: text(`${spec.id}.choice.take-risk`, "承担此险"), threatId: spec.risk, ...(repeatable(spec) ? { riskRepeatBehavior: "allow-repeat-resolution"          } : {}), outcomes: { success: { effects: safePrimary }, costlySuccess: { effects: safePrimary }, failure: { effects: declare([{ op: "ADD_CULTIVATION", amount: 60 }]) } } },
  ...
  const npcEffect               = declare(spec.participant === undefined ? [] : [{ op: "ADJUST_NPC_RELATION", actorBindingKey: "actor", affinityDelta: 6, trustDelta: 4, reasonTag: "npc.reason.event" }]);
  return [
...
const npcEvents         = [
  ...
  { id: "content01.pei.sparring-rain", title: "雨中试剑", summary: "雨线斜落，裴照川只问你是否还愿意拔剑，胜负之外还要看你如何收手。", actions: ["cultivate", "pursuit"], salience: 3, topic: "rivalry", continuity: ["sword", "promise"], build: "sword", npcRole: "sword", participant: core("pei-zhaochuan"), cooldown: spaced(4) , closure: true },
  ...
  { id: "content01.pei.promise-echo", title: "剑约未冷", summary: "多年后那柄断剑仍在，裴照川没有复述旧约，只把另一条路摆到你面前。", salience: 4, topic: "promise", continuity: ["sword", "rivalry"], build: "sword", npcRole: "sword", participant: core("pei-zhaochuan"), cooldown: spaced(4) , closure: true },
  ...
  { id: "content01.jiang.debt-echo", title: "旧账新页", summary: "姜雪芜翻到旧账那一页，没有催促，只问你如今是否仍认得当年的代价。", salience: 3, topic: "debt", continuity: ["medicine", "promise"], build: "alchemy", npcRole: "healer", participant: core("jiang-xuewu"), cooldown: spaced(4) , closure: true },
  ...
  { id: "content01.cen.shared-echo", title: "伤痕相认", summary: "旧伤在阴雨里同时发作，岑不归看见你按住肩头，便知道那一日没有被忘记。", salience: 3, topic: "injury", continuity: ["body", "memory"], build: "body", npcRole: "body", participant: core("cen-bugui"), cooldown: spaced(4) , closure: true },
  ...
  { id: "content01.xie.divided-spoils", title: "分利", summary: "所得不如预想，谢听潮仍按旧话分成，只把最后一件用途不明的东西留在中央。", actions: ["worldly"], salience: 3, topic: "trust", continuity: ["fortune", "promise"], build: "fortune", npcRole: "ruin-explorer", participant: core("xie-tingchao"), cooldown: spaced(4) , closure: true },
  { id: "content01.xie.map-echo", title: "图上旧折", summary: "秘图旧折痕与眼前山势重合，谢听潮没有催你，只把当初说过的话轻轻念了一遍。", salience: 4, topic: "secret", continuity: ["promise", "exploration"], build: "fortune", npcRole: "ruin-explorer", participant: core("xie-tingchao"), cooldown: spaced(4) , closure: true },
  ...
  { id: "content01.xu.empty-courtyard", title: "空院", summary: "院门仍旧，檐下却积了厚灰。邻人只说许长安早已离开，没有人知道他最后去了哪里。", actions: ["pursuit"], salience: 4, topic: "loss", continuity: ["time", "promise"], npcRole: "mortal", participant: core("xu-changan"), effects: [{ op: "SET_NPC_STATUS", actorBindingKey: "actor", targetStatus: "departed", revealToPlayer: true, reasonTag: "npc.reason.status" }], cooldown: spaced(4) , closure: true },
  { id: "content01.xu.promise-echo", title: "树下旧诺", summary: "旧树又添一圈年轮，你终于站回门前；许长安是否还在，已不再是唯一的问题。", salience: 4, topic: "promise", continuity: ["time", "memory"], npcRole: "mortal", participant: core("xu-changan"), cooldown: spaced(4) , closure: true }
];
...
const roadCauseEvents         = [
  ...
  { id: "content01.road.kindness-echo", title: "故人递伞", summary: "多年后雨又落下，一把伞从身侧递来；那张脸比记忆成熟，旧日一程仍被记得。", salience: 3, topic: "memory", continuity: ["trust", "travel"], cooldown: spaced(4) , closure: true },
  { id: "content01.road.conflict-echo", title: "旧路再逢", summary: "同一条窄路上，你再次看见熟悉身影；当年的争执已经长出新的分量。", salience: 3, topic: "rivalry", continuity: ["memory", "travel"], cooldown: spaced(4) , closure: true }
];

const CONTENT01_EVENTS                    = [...onboarding, ...ordinary, ...npcEvents, ...buildEvents, ...riskEvents, ...roadCauseEvents].map(event);

const CONTENT01_CAUSE_CHAINS = [
...
const CONTENT01_CAUSE_TEMPLATES = [
...
const CONTENT01_PACK              = sealContentPack({
...
module.exports = Object.assign({}, {
  ...
  CONTENT01_ZH_CN,
  CONTENT01_EVENTS,
  CONTENT01_CAUSE_CHAINS,
  CONTENT01_CAUSE_TEMPLATES,
  CONTENT01_PACK
});
