...
const ALLOWED_FIELDS = new Set(["id", "title", "phase", "verificationProfile", "executionProfile", "read", "optional_read", "requiredContracts", "optionalContracts", "blockers", "inspect_first", "in", "out", "allowedScope", "forbiddenScope", "accept", "tests"]);
...
export function parseTaskDefinition(text, source = "task.yaml") {
  ...
  const task = {
    taskId: result.id, title: result.title, phase: Number(result.phase), verificationProfile: result.verificationProfile ?? "standard", executionProfile: result.executionProfile ?? "default",
...
export function formatTaskContext(context) {
  const { task } = context; const lines = [`=== ${task.source} ===`, task.raw.trimEnd(), "", "=== task-context ===", `taskId: ${task.taskId}`, `phase: ${task.phase}`, `verificationProfile: ${task.verificationProfile}`, `executionProfile: ${task.executionProfile}`, `requiredContracts: [${task.requiredContracts.join(",")}]`, `optionalContracts: [${task.optionalContracts.join(",")}]`, `blockers: [${task.blockers.join(",")}]`, `allowedScope: [${task.allowedScope.join(" | ")}]`, `forbiddenScope: [${task.forbiddenScope.join(" | ")}]`];
