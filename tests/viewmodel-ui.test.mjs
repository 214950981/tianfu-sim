...
function active(content = contentRegistry()) {
  ...
  return validateGameState({ ...started, run: { ...started.run, events: { history: [{ eventId: "past.event", nodeIndex: 0, resultTier: "success" }], current: { eventId: "dev.first-choice", kind: "choice" } }, causes: { byId: { hidden: cause("hidden", "hidden"), hinted: cause("hinted", "hint"), explicit: cause("explicit", "journal") } }, npcs: { nextNpcSequence: 1, byId: { secretNpc: { npcId: "secretNpc", definitionId: "dev.mysterious-mentor", originKind: "core", displayName: "Secret", traitTags: ["npc.trait.reserved"], factIds: ["npc.fact.identity"], tags: ["mentor"], roleTags: ["mentor"], actualStatus: "dead", relation: { affinity: 0, trust: 0, debt: 0, encounterCount: 0 }, significance: 99, promotedToA: false, knowledge: { met: false, knownFactIds: [], knownTraitTags: [] }, createdAge: 20, createdNodeIndex: 0, lastEncounterAge: 20, lastEncounterNodeIndex: 0, encounterCount: 0, milestoneFacts: [] } }, roleIndex: { mentor: ["secretNpc"] } } } });
}
