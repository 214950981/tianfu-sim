...
test("verificationProfile is accepted as scalar task metadata", () => { const task = parseTaskDefinition("id: FAST01\ntitle: fast\nphase: 2\nverificationProfile: fast-lane\nrequiredContracts: []\n", "fast.yaml"); assert.equal(task.verificationProfile, "fast-lane"); assert.ok(formatTaskContext({ task, blockerEntries: [], loadedContracts: [], optionalContracts: [] }).includes("verificationProfile: fast-lane")); });
test("executionProfile is accepted as scalar task metadata", () => { const task = parseTaskDefinition("id: CREDIT01\ntitle: credit\nphase: 2\nverificationProfile: fast-lane\nexecutionProfile: credit-efficient\nrequiredContracts: []\n", "credit.yaml"); assert.equal(task.executionProfile, "credit-efficient"); assert.ok(formatTaskContext({ task, blockerEntries: [], loadedContracts: [], optionalContracts: [] }).includes("executionProfile: credit-efficient")); });
