...
test("LOOPFIX02A-R1-001: a selected max-occurrences Event accepts its legal current choice exactly once", () => {
  const pack = recurringPack({ op: "ADD_CULTIVATION", amount: 1 }, "selected-once", {}, { maxOccurrences: 1 }); const content = registry(pack); const state = active(content, pack.manifest.contentVersion);
  const selected = reduce({ state, command: { type: "CHOOSE_ACTION", actionId: "travel" }, context: { rulesVersion: state.rulesVersion, contentVersion: state.contentVersion, content, commandId: "cmd:selected-once:action" } });
  const event = content.getEvent(state.contentVersion, "dev.first-choice"); assert.equal(selected.state.run.events.current?.eventId, event.id); assert.equal(selected.state.run.events.occurrences[event.id]?.occurrenceCount, 1);
  const completed = reduce({ state: selected.state, command: { type: "CHOOSE_EVENT_OPTION", eventId: event.id, optionId: "repeat" }, context: { rulesVersion: state.rulesVersion, contentVersion: state.contentVersion, content, commandId: "cmd:selected-once:choice" } });
  assert.equal(completed.state.run.events.current, undefined); assert.equal(completed.state.run.events.occurrences[event.id]?.occurrenceCount, 1); assert.equal(isEventEligible(event, completed.state), false);
  const before = structuredClone(selected.state); assert.throws(() => reduce({ state: selected.state, command: { type: "CHOOSE_EVENT_OPTION", eventId: "dev.ordinary-fallback", optionId: "repeat" }, context: { rulesVersion: state.rulesVersion, contentVersion: state.contentVersion, content, commandId: "cmd:selected-once:wrong-event" } })); assert.throws(() => reduce({ state: selected.state, command: { type: "CHOOSE_EVENT_OPTION", eventId: event.id, optionId: "missing" }, context: { rulesVersion: state.rulesVersion, contentVersion: state.contentVersion, content, commandId: "cmd:selected-once:wrong-choice" } })); assert.deepEqual(selected.state, before);
});

test("LOOPFIX02A-R1-002: a selected cooldown Event completes, stays blocked, then reopens at the exact boundary", () => {
  const pack = recurringPack({ op: "ADD_CULTIVATION", amount: 1, repeatBehavior: "allow-cumulative" }, "selected-cooldown", {}, { minNodesBetween: 2 }); const content = registry(pack); const state = active(content, pack.manifest.contentVersion);
  const selected = reduce({ state, command: { type: "CHOOSE_ACTION", actionId: "travel" }, context: { rulesVersion: state.rulesVersion, contentVersion: state.contentVersion, content, commandId: "cmd:selected-cooldown:action" } }); const event = content.getEvent(state.contentVersion, "dev.first-choice");
  const completed = reduce({ state: selected.state, command: { type: "CHOOSE_EVENT_OPTION", eventId: event.id, optionId: "repeat" }, context: { rulesVersion: state.rulesVersion, contentVersion: state.contentVersion, content, commandId: "cmd:selected-cooldown:choice" } });
  assert.equal(completed.state.run.events.occurrences[event.id]?.occurrenceCount, 1); assert.equal(isEventEligible(event, completed.state), false); assert.equal(isEventEligible(event, occurrenceState(completed.state, event.id, 1, selected.state.run.nodeIndex, selected.state.run.nodeIndex + 2)), false); assert.equal(isEventEligible(event, occurrenceState(completed.state, event.id, 1, selected.state.run.nodeIndex, selected.state.run.nodeIndex + 3)), true);
});

test("LOOPFIX02A-R1-003: choice retry, Snapshot and replay preserve one occurrence, hash and RNG", async () => {
  const pack = recurringPack({ op: "ADD_CULTIVATION", amount: 1 }, "choice-replay", {}, { maxOccurrences: 1 }); const content = registry(pack); const initial = active(content, pack.manifest.contentVersion); const action = { commandId: "cmd:choice-replay:action", playerId: initial.run.playerId, runId: initial.run.runId, expectedStateVersion: initial.stateVersion, rulesVersion: initial.rulesVersion, contentVersion: initial.contentVersion, clientPlatform: "dev", clientBuild: "recurrence", command: { type: "CHOOSE_ACTION", actionId: "travel" } };
  const selected = executeLoggedCommand(initial, createCommandLog(initial), action, {}, content); const choice = { ...action, commandId: "cmd:choice-replay:choice", expectedStateVersion: selected.output.state.stateVersion, command: { type: "CHOOSE_EVENT_OPTION", eventId: "dev.first-choice", optionId: "repeat" } }; const completed = executeLoggedCommand(selected.output.state, selected.commandLog, choice, {}, content); const replayed = replayCommandLog({ initialState: initial, commandLog: completed.commandLog, content }); const snapshot = validateSnapshot(JSON.parse(JSON.stringify(createSnapshot(completed.output.state, 2))));
  assert.equal(completed.output.state.run.events.occurrences["dev.first-choice"]?.occurrenceCount, 1); assert.equal(replayed.finalRuleStateHash, ruleStateHash(completed.output.state)); assert.equal(snapshot.ruleStateHash, replayed.finalRuleStateHash); assert.deepEqual(replayed.state.run.rng, completed.output.state.run.rng);
  const store = new InMemoryGatewayStore(); store.seedRun(initial); const gateway = new CommandGateway({ store, content }); await gateway.sendCommand({ playerId: initial.run.playerId }, action); const gatewayChoice = { ...choice, expectedStateVersion: store.readRun(initial.run.runId).state.stateVersion }; const first = await gateway.sendCommand({ playerId: initial.run.playerId }, gatewayChoice); const retry = await gateway.sendCommand({ playerId: initial.run.playerId }, gatewayChoice); assert.deepEqual(retry, first); assert.equal(store.readRun(initial.run.runId).state.run.events.occurrences["dev.first-choice"]?.occurrenceCount, 1);
});
