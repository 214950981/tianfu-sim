...
test("UI04E_flow: terminal transitions advance ENDING -> LIFE_BOOK -> REBIRTH_RESULT -> NEXT_LIFE in order", async () => {
  ...
  // UI04E-R1: the success envelope carries no next-run identifiers. The next life is created by an
  // explicit, client-driven createRunOffer call, not by this RPC.
  assert.equal(nl.nextBootstrapId, undefined, "the success envelope must not carry nextBootstrapId");
  assert.equal(nl.nextRunId, undefined, "the success envelope must not carry nextRunId");
  ...
  // UI04E-R1: NO new run document and NO new bootstrap mapping were written by advanceTerminal.
  // The runs collection only contains the original `runId`; bootstraps only carries the original entry.
  const runs = setup.database.snapshot().get("tianfu2_runs");
  assert.equal(runs.size, 1, "advanceTerminal must NOT create a new run document on NEXT_LIFE");
...
test("UI04E_host: the committed cloud function drives START_RUN -> real action -> dying -> ENDING -> LIFE_BOOK -> REBIRTH_RESULT -> NEXT_LIFE", async () => {
  ...
  assert.equal(nl.ok, true, JSON.stringify(nl));
  assert.equal(nl.stage, "NEXT_LIFE");
  // UI04E-R1: the cloud host must NOT return nextBootstrapId/nextRunId; arriving at NEXT_LIFE is a pure
  // sidecar transition. The next life is created by an explicit createRunOffer call afterwards.
  assert.equal(nl.nextBootstrapId, undefined, "the host must not publish nextBootstrapId");
  assert.equal(nl.nextRunId, undefined, "the host must not publish nextRunId");

  // The prior run remains at NEXT_LIFE. UI04E-R1: no new run document was created by advanceTerminal.
...
test("UI04E_host: an exact retry after a post-commit timeout settles exactly once and recovers the same NEXT_LIFE stage", async () => {
  const database = createFakeCloudDatabase();
  const handler = loadCloudFunction({ openid: ALICE, database });
  const registry = createLiveContentRegistry();
  ...
  // UI04E-R1: the receipt no longer carries next-run identifiers.
  assert.equal(firstNext.nextBootstrapId, undefined, "the receipt must not carry nextBootstrapId");
  assert.equal(firstNext.nextRunId, undefined, "the receipt must not carry nextRunId");
  ...
  assert.equal(retried.stage, "NEXT_LIFE");
...
test("UI04E_page: the v2-live page renders all four terminal stages and never locally skips an authoritative stage", async () => {
  ...
  // UI04E-R1: NEXT_LIFE settle carries NO next-run identifiers. The next life is started by an
  // explicit `startNextLife(persistedPendingBootstrapId)` call from the NEXT_LIFE page CTA.
  assert.equal(nl.nextBootstrapId, undefined, "the controller must not surface nextBootstrapId");
  assert.equal(nl.nextRunId, undefined, "the controller must not surface nextRunId");
  // The controller's current runId is still the OLD dying run after the NEXT_LIFE advance — arrival
  // at NEXT_LIFE does NOT auto-switch the run.
  assert.equal(controller.view().state.runId, runId, "arriving at NEXT_LIFE must not switch the controller's current runId");

  // The explicit start-next-life action: the caller persists a pending bootstrap id and hands it to
  // the controller. The controller then calls createRunOffer and switches to the returned run.
  const pendingBootstrapId = "next-life:" + Date.now().toString(36);
  ...
  assert.notEqual(controller.view().state.runId, runId, "after startNextLife the controller's runId is the new run");
  assert.equal(nextView.state.playerId === undefined || nextView.state.runId !== runId, true);
...
test("UI04E_protocol: parseAdvanceTerminalResult accepts a NEXT_LIFE settle and refuses a secret-bearing one", () => {
  const good = { ok: true, terminalTransitionId: "term:1", stage: "NEXT_LIFE", version: 3 };
  const parsed = parseAdvanceTerminalResult(good);
  assert.deepEqual(parsed, { ok: true, terminalTransitionId: "term:1", stage: "NEXT_LIFE", version: 3 });
  // UI04E-R1: a hostile/legacy server that injects nextBootstrapId/nextRunId is ignored, not surfaced.
  const withLegacy = { ok: true, terminalTransitionId: "term:1", stage: "NEXT_LIFE", version: 3, nextBootstrapId: "boot:next", nextRunId: "run-next" };
  const parsedLegacy = parseAdvanceTerminalResult(withLegacy);
  assert.deepEqual(parsedLegacy, { ok: true, terminalTransitionId: "term:1", stage: "NEXT_LIFE", version: 3 });
  assert.equal(parsedLegacy.nextBootstrapId, undefined);
  assert.equal(parsedLegacy.nextRunId, undefined);
...
test("UI04E_page_source: v2-live handles the four terminal stages and uses a literal require of the runtime", () => {
  ...
  // UI04E-R1: the page now exposes an explicit NEXT_LIFE start CTA + pending-bootstrap lifecycle.
  assert.equal(source.includes("onStartNextLife"), true, "the page must expose an explicit start-next-life CTA");
  assert.equal(source.includes("persistNextLifePendingBootstrap"), true, "the page must persist a pending next-life bootstrap key");
  assert.equal(source.includes("promoteNextLifeBootstrap"), true, "the page must promote the pending key only after createRunOffer succeeds");
  assert.equal(source.includes("readNextLifePendingBootstrap"), true, "the page must reuse an existing pending key on retry/reload");
  ...
  // UI04E-R1: the NEXT_LIFE block must expose the explicit CTA, not auto-start the next life.
  assert.equal(wxml.includes("onStartNextLife"), true, "v2-live.wxml must bind the explicit NEXT_LIFE CTA to onStartNextLife");
