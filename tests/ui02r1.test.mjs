 ...
 *
 * UI03 is such a task for four entries, all updated on purpose and disclosed in its LAST_RESULT:
 *   - `server/src` — the current interaction now publishes the authoritative current-event id, because
 *     the COMMAND contract keys CHOOSE_EVENT_OPTION by `eventId` and `interactionId` is an *instance*
 *     identity, so the public ViewModel could not construct the contract's own command before;
 *   - `packages/platform-contract/src` — the matching optional field on CurrentInteraction;
 *   - `packages/wechat-shell/src` — the UI03 live session controller (WeChatRunController);
 *   - `miniprogram/pages/v2-preview/v2-fixtures.json` — regenerated by the real production chain, which
